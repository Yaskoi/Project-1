...
assets = ['NVDA', 'AAPL', 'PLTR', 'TSLA', 'MRNA', 'PFE', 'ILMN', 
          ...
          'TLT', 'BND', 'BTC-USD', 'ETH-USD', '005930.KS', 
          'NSRGY', 'TM', 'RIO', 'TCEHY', 'KO', 'ENB']

# Télécharger les données des actifs sur Yahoo Finance
data = yf.download(assets, interval='1h', start="2023-01-01", end="2023-07-01")['Close']
...
weights = np.array([0.05, 0.04, 0.03, 0.05, 0.03, 0.03, 0.03, 
                    0.02, 0.02, 0.02, 0.03, 0.03, 0.05, 
                    0.03, 0.04, 0.04, 0.03, 0.03, 0.02, 
                    0.03, 0.02, 0.05, 0.04, 0.02, 
                    0.03, 0.03, 0.03, 0.03, 0.02, 0.03]) # Somme = 1
...
print(f"Valeur finale du portefeuille ML en Stratégie : {(1 + total_return_strategy) * 1000:.2F}")
...
print(f"Valeur finale du portefeuille ML en Buy & Hold : {(1 + total_return_buy_hold) * 1000:.2F}")
